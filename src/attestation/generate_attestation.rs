...
pub fn attestation(
    ...
    let day = format!("Le : {}/{}/{}            {}",now.day(),now.month(),now.year(),time);
    ...
    let font3 = doc
        ...
        .unwrap();
    current_layer.use_text("ATTESTATION DE DÉPLACEMENT DÉROGATOIRE", 14, Mm(45.0), Mm(280.0), &font);
    ...
    current_layer.add_line_break();
    current_layer.write_text("hors de leur domicile, d’un document leur permettant de justifier que le déplacement considéré entre dans le champ de l’une de ces exception",&font3);
    current_layer.add_line_break();
    ...
    match choice {
        1 => {
            ...
            current_layer.add_line_break();
            current_layer.write_text("pouvant être différés;déplacements pour un concours ou un examen;",&font3);
        }
        2 => {
        ...
        3 => {
            current_layer.write_text("Consultations, examens et soins ne pouvant être assurés à distance et achats",&font3);
            current_layer.add_line_break();
            current_layer.write_text("de médicaments;",&font3);
        }
        4 => {
            current_layer.write_text("Déplacements pour motif familial impérieux, pour l’assistance auxpersonnes",&font3);
            current_layer.add_line_break();
            current_layer.write_text("vulnérables et précaires ou la garde d’enfants;",&font3);
        }
        5 => {
            current_layer.write_text("Déplacements des personnes en situation de handicap et leur accompagnant;",&font3);
        }
        6 => {
            current_layer.write_text("Déplacements en plein air ou vers un lieu de plein air, sans changement ",&font3);
            current_layer.add_line_break();
            current_layer.write_text("du lieu de résidence, dans la limite de trois heures quotidiennes et dans",&font3);
            current_layer.add_line_break();
            current_layer.write_text("un rayon maximal de vingt kilomètres autour du domicile, liés soit à ",&font3);
            current_layer.add_line_break();
            current_layer.write_text("l’activité physique ou aux loisirs individuels, à l’exclusion de toute",&font3);
            current_layer.add_line_break();
            current_layer.write_text("pratique sportive collective et de toute proximité avec d’autres personnes,",&font3);
            current_layer.add_line_break();
            current_layer.write_text("soit à la promenade avec les seules personnes regroupées dans un même",&font3);
            current_layer.add_line_break();
            current_layer.write_text("domicile, soit aux besoins des animaux de compagnie;",&font3);
        }
        7 => {
            current_layer.write_text("Convocations judiciaires ou administratives et déplacements pour se rendre",&font3);
            current_layer.add_line_break();
        ...
        9 => {
            current_layer.write_text("Déplacements pour chercher les enfants à l’école et à l’occasion de leurs ",&font3);
            current_layer.add_line_break();
    ...
    current_layer.begin_text_section();
    current_layer.set_font(&font3, 14);
    current_layer.set_text_cursor(Mm(110.0), Mm(50.0));
    current_layer.set_line_height(20);
    current_layer.set_word_spacing(3000);
    ...
    current_layer.begin_text_section();
    current_layer.set_font(&font3, 11);
    current_layer.set_text_cursor(Mm(10.0), Mm(30.0));
    current_layer.set_line_height(14);
    current_layer.set_word_spacing(3000);
    ...
    doc.save(&mut BufWriter::new(File::create("attestation.pdf").unwrap())).unwrap();
}